...
describe("pricing config", () => {
  ...
  test("pro tier prices match SSOT", () => {
    expect(pricing.pro.monthlyPrice).toBe(1500);
    expect(pricing.pro.yearlyPrice).toBe(14400);
  });

  test("team tier prices match SSOT", () => {
    expect(pricing.team.monthlyPrice).toBe(3900);
    expect(pricing.team.yearlyPrice).toBe(38400);
  });
