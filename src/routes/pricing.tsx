...
const FREE_PRICE: Price = {
  ...
  product: {
    id: "free-product",
    name: "Free",
    description: "Start for free with managed credits or your own keys.",
    marketing_features: [
      { name: "10,000 requests per month" },
      { name: "3 API keys" },
...
const TEAM_FEATURES = [
  ...
  { name: "Pay-as-you-go managed credits (15% margin)" },
  { name: "Unlimited API keys" },
  { name: "Smart routing (cost, quality, latency)" },
  { name: "All modalities (STT, TTS, images, embeddings)" },
  { name: "1-year analytics retention" },
      ...
      <CardHeader className="bg-muted pb-3 lg:min-h-50.5 dark:bg-surface-elevated">
        ...
        {isCurrentPlan ? (
          ...
          <Button variant="solid" asChild>
            <a href="mailto:sales@omni.dev">Contact Sales</a>
          </Button>
        )}
      </CardHeader>
...
export const Route = createFileRoute("/pricing")({
  ...
  loader: async () => {
    // Only fetch prices when billing is configured; otherwise checkout is
    // unavailable and paid tiers degrade to a Contact Sales CTA
    const [prices, sessionData] = await Promise.all([
      hasBilling ? getPrices() : Promise.resolve([] as Price[]),
      fetchSession(),
    ]);
